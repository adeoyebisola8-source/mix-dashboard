module.exports = async (req, res) => {
  ...
  try {
    ...
    const projects = [
      ...
      { name: 'Football Frenzy', token: '259ebd6d27529f40dc31376f3eca4545', secret: '66bd49b335376a3e7cae8a536e0d7134', signupEvent: null }
    ];
    ...
    for (let i = 0; i < projects.length; i++) {
      ...
      try {
        // Fetch events using token and Basic Auth
        ...
        if (!response.ok) {
          throw new Error(`Mixpanel API returned ${response.status} for ${project.name}`);
        }
        ...
        const churnRate = Math.round(100 - stickiness);
        ...
        data[i] = {
          ...
          churnRate: Math.max(churnRate, 5),
